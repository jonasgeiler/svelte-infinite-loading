...
<svelte:head>
	<title>Hacker News with Top Direction | svelte-infinite-loading</title>
</svelte:head>

<div id="hacker-news-with-top-direction-demo" class="demo-page flex-1 flex flex-column">
	<header class="primary">
		<nav>
			<i class="border white-border" aria-hidden="true">
				<img src={asset('/y18.svg')} alt="Hacker News Logo" />
			</i>
			<h5>Hacker News with Top Direction</h5>
		</nav>
	</header>

	<InfiniteLoading forceUseInfiniteWrapper onInfinite={infiniteHandler} direction="top" />

	{#each list as item, index (item.objectID)}
		<article class="hacker-news-item margin" data-num={list.length - index}>
			<div class="truncate">
				<a
					class="inline link"
					href={item.url ||
...
<style>
	:global(body) #hacker-news-with-top-direction-demo {
		--primary: #a33e00;
		--on-primary: #ffffff;
	...
	:global(body.dark) #hacker-news-with-top-direction-demo {
		--primary: #ffb596;
		--on-primary: #581e00;
